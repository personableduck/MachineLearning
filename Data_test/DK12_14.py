...
with open('C:/Users/1234/Downloads/(Input)Individual_Play_Data_KYWA (1).csv', 'rb') as f:
    reader = csv.reader(f)
    your_list = list(reader)

with open('C:/Users/1234/Downloads/GHSD_Graduates_Statistics.csv', 'rb') as t:
    reader2 = csv.reader(t)
    score_list = list(reader2)

with open('C:/Users/1234/Downloads/average_list.csv', 'rb') as k:
    reader3 = csv.reader(k)  
    hanna_list = list(reader3)
...
for index in range (1,len(score_list)):
    score_list_dlt.append(score_list[index])
...
for index in range (len(score_list_dlt)):
    only_performance.append(float(score_list_dlt[index][6]))
    
only_performance.sort()
#-----------------------------
    
only_score=[]

for index in range (len(your_list_dlt)):
    only_score.append(float(your_list_dlt[index][1]))

only_score.sort()

#------------------------------

picture_mat_x = range(int(len(only_performance)));

picture_x=[]

for index in range (len(picture_mat_x)):
    picture_x.append(int(picture_mat_x[index])+1)

plt.figure(0) 
plt.plot(picture_x,only_performance,'bo-')  
