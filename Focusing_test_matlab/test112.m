...
count=count+1;
z1(count)=z*10;
rec_r=Prop_SSA_v2(test_img,1.12,1.12,-z1(count),624/1000); %reconstruction
    if focus_type == 0
...
filter_size = bwareafilt(object_binary,[4 36]);
...
rec_r=Prop_SSA_v2(test_img,1.12,1.12,-z1(count),624/1000); 
    if focus_type == 0
        focus_img=abs(rec_r); %amplitude 
    elseif focus_type == 1  
        focus_img=angle(rec_r); %phase 
...
rec_r=Prop_SSA_v2(test_img,1.12,1.12,-z1(h),624/1000); %reconstruction
if focus_type == 0
    focus_img=abs(rec_r); %amplitude 
elseif focus_type == 1  
    focus_img=angle(rec_r); %phase 
