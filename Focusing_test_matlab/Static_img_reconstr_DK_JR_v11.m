...
lambda = 639e-9; % m // Justin's LED: 633nm //New fibered LED (Chris'): 624nm //Max fiber:625nm//Laser Diode:639nm
...
lowest_depth = 300; % um 300 ~ 500 before
highest_depth = 500; % um 
focus_type = 0; %focus_type // 0:amplitude focus, 1:phase focus

K=0.2; %fraction of added bands
...
axis equal
set(gcf,'Color',[1 1 1]);
z0(incr) = Contrast_Auto_focus_dk(crop_img(:,:,incr),focus_type,lowest_depth,highest_depth,pixelsize,lambda_V); % um

title(['depth is ', num2str(z0(incr)) ,'um']); 
zp=['Crop field ' num2str(incr) '/' num2str(Nb_ROI) ': depth is ',num2str(z0(incr)) 'um'];
disp(zp)
    end
end
...
figure(4445),imshow(mat_z*1e6,[])
truesize(gcf,[1000 1000]) %Adjust display size
incr3=0;
for i_r=1:divide_ROI
    for j_r=1:divide_ROI
            incr3=incr3+1;
            text(0.8+(j_r-1),i_r, [num2str(mat_z(incr3),'%0.2f'),' um'],'Color','r','FontSize',12);
    end
end
title('Distribution of z2 distances (um)')
