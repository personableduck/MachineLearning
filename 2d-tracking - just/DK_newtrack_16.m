                                                                                                                            ...
                                                                                                                            if(duplecount ~= 0)
                                                                                                                                ...
                                                                                                                                if( duplehappen == 1)
                                                                                                                                    ...
                                                                                                                                    for check_duple_loop=1:duplecount
                                                                                                                                        ...
                                                                                                                                        fast_total_coor{1,duplication_number(check_duple_loop,1)}(fast_obj_start+1,1) = nan;
                                                                                                                                        fast_total_coor{1,duplication_number(check_duple_loop,1)}(fast_obj_start+1,2) = nan;      
                                                                                                                                        fast_total_coor{1,duplication_number(check_duple_loop,1)}(fast_obj_start+1,3) = duplication_number(check_duple_loop,1);

                                                                                                                                        fast_total_coor{1,duplication_number(check_duple_loop,1)}(fast_obj_start+2,1) = nan;
                                                                                                                                        fast_total_coor{1,duplication_number(check_duple_loop,1)}(fast_obj_start+2,2) = nan;        
                                                                                                                                        fast_total_coor{1,duplication_number(check_duple_loop,1)}(fast_obj_start+2,3) = duplication_number(check_duple_loop,1);

                                                                                                                                        fast_total_coor{1,duplication_number(check_duple_loop,1)}(fast_obj_start+3,1) = nan;
                                                                                                                                        fast_total_coor{1,duplication_number(check_duple_loop,1)}(fast_obj_start+3,2) = nan;       
                                                                                                                                        fast_total_coor{1,duplication_number(check_duple_loop,1)}(fast_obj_start+3,3) = duplication_number(check_duple_loop,1);

                                                                                                                                        fast_total_coor{1,duplication_number(check_duple_loop,1)}(fast_obj_start+4,1) = nan;
                                                                                                                                        fast_total_coor{1,duplication_number(check_duple_loop,1)}(fast_obj_start+4,2) = nan;
                                                                                                                                        fast_total_coor{1,duplication_number(check_duple_loop,1)}(fast_obj_start+4,3) = duplication_number(check_duple_loop,1); 

                                                                                                                                        fast_total_coor{1,duplication_number(check_duple_loop,1)}(fast_obj_start+5,1) = nan;
                                                                                                                                        fast_total_coor{1,duplication_number(check_duple_loop,1)}(fast_obj_start+5,2) = nan;
                                                                                                                                        fast_total_coor{1,duplication_number(check_duple_loop,1)}(fast_obj_start+5,3) = duplication_number(check_duple_loop,1);
