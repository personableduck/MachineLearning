        ...
        if(check_f1 == 1)
            ...
            if (check_dist == 1) %first distance threshold check
                ...
                if(fast_obj_start == 1)  
                    ...
                    for try_loop = 1: row_dist_size(1,1) 
                        angle_fast_test = atan((xyz{1,fast_obj_start+1}(distance_fast_mat(row_dist(try_loop),2),2)-yc_1)/(xyz{1,fast_obj_start+1}(distance_fast_mat(row_dist(try_loop),2),1)-xc_1))*180/pi; % unit is degree   
                        ...
                        if( (xyz{1,fast_obj_start+1}(distance_fast_mat(row_dist(try_loop),2),2)-yc_1) == 0 & (xyz{1,fast_obj_start+1}(distance_fast_mat(row_dist(try_loop),2),1)-xc_1) == 0 )
                            priority_angle_order(try_loop,1) = priority_zero; 
                        end
                ...
                for try_loop2 = 1: row_dist_size(1,1)
                    if(stop_search == 0) 
                        ...
                        if(fast_obj_start == 1)
                            ...
                            if(posit_nega == posit_nega2 | posit_nega2 == 0 | posit_nega == 0) %%%%%%% need to change here
                                ...
                                if( cont_rest_th(inside_loop,1) > decide_slow_angle)           
                                    if(abs(cont_angle_th(inside_loop,1)) > tangent_control | abs(angle_fast_test) > tangent_control)              
                                        if(abs(cont_angle_th(inside_loop,1))+control_vecter_angle > abs(atan((yc_2-yc_1)/(xc_2-xc_1))*180/pi) & abs(cont_angle_th(inside_loop,1))-control_vecter_angle < abs(atan((yc_2-yc_1)/(xc_2-xc_1))*180/pi)) % unit is degree
                                            pass_numfil=1;
                                        end
                                    else
                                        if(cont_angle_th(inside_loop,1)+control_vecter_angle > atan((yc_2-yc_1)/(xc_2-xc_1))*180/pi & cont_angle_th(inside_loop,1)-control_vecter_angle < atan((yc_2-yc_1)/(xc_2-xc_1))*180/pi) % unit is degree
                                        pass_numfil=1;
                                        end
                                    end    
                                else
                                    if(abs(cont_angle_th(inside_loop,1)) > tangent_control | abs(angle_fast_test) > tangent_control)              
                                        if(abs(cont_angle_th(inside_loop,1))+control_vecter_angle*multilple_short_angle > abs(atan((yc_2-yc_1)/(xc_2-xc_1))*180/pi) & abs(cont_angle_th(inside_loop,1))-control_vecter_angle*multilple_short_angle < abs(atan((yc_2-yc_1)/(xc_2-xc_1))*180/pi)) % unit is degree
                                            pass_numfil=1;
                                        end
                                    else
                                        if(cont_angle_th(inside_loop,1)+control_vecter_angle*multilple_short_angle > atan((yc_2-yc_1)/(xc_2-xc_1))*180/pi & cont_angle_th(inside_loop,1)-control_vecter_angle*multilple_short_angle < atan((yc_2-yc_1)/(xc_2-xc_1))*180/pi) % unit is degree
                                        pass_numfil=1;
                                        end
                                    end
                                end                                   
                        ...
                        if(fast_obj_start == 1)                         
                            ...
                            if( posit_nega2 == 0 | posit_nega == 0 )
                                if((distance_fast_mat(row_dist(priority_angle_order(try_loop2,2)))+cont_rest_th(inside_loop,1))*increase_distance_threshold < sqrt( (xc_2-xc_back_1)^2+(yc_2-yc_back_1)^2 ))
                                    increase_check=0; 
                                else   
                                    increase_check=1;
                                end    
                            else
                                if((distance_fast_mat(row_dist(priority_angle_order(try_loop2,2)))+cont_rest_th(inside_loop,1))*increase_distance_threshold < sqrt( (xc_2-xc_1)^2+(yc_2-yc_1)^2 ))
                                    increase_check=0; 
                                else   
                                    increase_check=1;
                                end        
                                ...
                                if(check_f2 == 1)
                                    ...
                                    if (check_secd == 1)
                                        ...
                                        for find_secd_loop = 1: row_secd_size(1,1)
                                            angle_secd_fast_test = atan((xyz{1,fast_obj_start+2}(distance_repeat_rest(row_secd(find_secd_loop),2),2)-yc_2)/(xyz{1,fast_obj_start+2}(distance_repeat_rest(row_secd(find_secd_loop),2),1)-xc_2))*180/pi;
                                            ...
                                            if( (xyz{1,fast_obj_start+2}(distance_repeat_rest(row_secd(find_secd_loop),2),2)-yc_2) == 0 & (xyz{1,fast_obj_start+2}(distance_repeat_rest(row_secd(find_secd_loop),2),1)-xc_2) == 0 )
                                            ...
                                            if(stop_search == 0)
                                                ...
                                                if( fast_obj_start > 1)
                                                    if( posit_nega2 == 0 | posit_nega == 0 )
                                                        if(angle_fast <= 45 & angle_fast >= -45) %vetorc
                                                            vector=1; % x vetorc
                                                            diff_vector = xc_2-xc_back_1;
                                                            posit_nega= sign(diff_vector); % x_vector>0 : 1 , x_vector<0 : -1 , 0=0
                                                        else 
                                                            vector=0; % y vetorc
                                                            diff_vector = yc_2-yc_back_1;
                                                            posit_nega=sign(diff_vector); % y_vector>0 : 1 , y_vector<0 : -1 , 0=0
                                                        end
                                                    else
                                                         if(angle_fast <= 45 & angle_fast >= -45) %vetorc
