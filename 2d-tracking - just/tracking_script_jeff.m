...
rec2 = Prop_SSA(avg_holo,pixelsize/interp_factor,pixelsize/interp_factor,...
...
%         rec = angle(rec);
        ...
        recon_stack(:,:,iter) = rec;
        recon_diff_avg(:,:,iter) = rec - recon_avg_holo;
...
recon_diff_avg_BW = zeros(height,width,length_frame_stack);
black_count = zeros(1,length_frame_stack);
%thresholdValue = 0.65;
...
%end
 ...
 %figure
    ...
    figure
    imshow(abs(recon_diff_avg(:,:,1)),[]);
    title('recon_stack[1]-recon_avg')
        figure
    imshow(abs(recon_diff_avg(:,:,2)),[]);
    title('recon_stack[2]-recon_avg')
        figure
    imshow(abs(recon_diff_avg(:,:,3)),[]);
    title('recon_stack[3]-recon_avg')
