function ans_xyz= change_first_start_point(xyz_new_check,xyz,fast_obj_start,inside_loop,increase_number,distance_threshold_fast,priority_degree_rate,priority_zero)

count_passe=0;
distance_fast_mat=0;    
[row_f1,col_f1,check_f1]=find( xyz{1,fast_obj_start+increase_number}(:,1) < xyz_new_check(inside_loop,1) + distance_threshold_fast & xyz{1,fast_obj_start+increase_number}(:,1) > xyz_new_check(inside_loop,1) - distance_threshold_fast);   
xc_1=xyz_new_check(inside_loop,1);
yc_1=xyz_new_check(inside_loop,2);
