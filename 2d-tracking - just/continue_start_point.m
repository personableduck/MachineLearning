function ans_xyz= continue_start_point(origin_size_fst,xyz,fast_total_coor,fast_obj_start,inside_loop,increase_number,distance_threshold_fast,priority_degree_rate,threshold_diff_dislocation,priority_zero,decide_slow,decide_slow_angle,tangent_control,control_vecter_angle,multilple_short_angle,increase_distance_threshold)
...
if( estm_ft_size(1,1) >= origin_size_fst)
    [row_f1,col_f1,check_f1]=find( xyz{1,fast_obj_start+increase_number}(:,1) < fast_total_coor{1,inside_loop}(origin_size_fst,1) + distance_threshold_fast & xyz{1,fast_obj_start+increase_number}(:,1) > fast_total_coor{1,inside_loop}(origin_size_fst,1) - distance_threshold_fast);
    xc_1=fast_total_coor{1,inside_loop}(origin_size_fst,1);
    yc_1=fast_total_coor{1,inside_loop}(origin_size_fst,2);
    xc_back_1=fast_total_coor{1,inside_loop}(origin_size_fst-1,1);
    yc_back_1=fast_total_coor{1,inside_loop}(origin_size_fst-1,2);
    xc_back_2=fast_total_coor{1,inside_loop}(origin_size_fst-2,1);
    yc_back_2=fast_total_coor{1,inside_loop}(origin_size_fst-2,2);
end
