function ans_xyz3= slow_third_point(xc_1,yc_1,xc_2,yc_2,xc_3,yc_3,angle_secd_fast,dist_thresh_third,decide_slow,threshold_diff_dislocation,xyz_11,fast_obj_start,inside_loop,increase_number,distance_threshold_fast,priority_degree_rate,increase_distance_threshold,decide_slow_angle,tangent_control,control_vecter_angle,multilple_short_angle,priority_zero)

count_passe=0;
distance_thrd_mat=0;
[row_f3,col_f3,check_f3]=find( xyz_11{1,fast_obj_start+2}(:,1) < xc_3 + distance_threshold_fast & xyz_11{1,fast_obj_start+2}(:,1) > xc_3 - distance_threshold_fast);       
...
if(check_f3 == 1)
    ...
    for dist_thrd_loop = 1: maxsize_fast_xyz3(1,1)                                   
        xx_3 = xyz_11{1,fast_obj_start+2}(row_f3(dist_thrd_loop),1) - xc_3;
        yy_3 = xyz_11{1,fast_obj_start+2}(row_f3(dist_thrd_loop),2) - yc_3;
    ...
    if (check_third == 1)
        ...
        for find_rest_loop = 1: row_third_size(1,1)
            angle_rest_th_test=atan((xyz_11{1,fast_obj_start+2}(distance_thrd_mat(row_third(find_rest_loop),2),2)-yc_2)/(xyz_11{1,fast_obj_start+2}(distance_thrd_mat(row_third(find_rest_loop),2),1)-xc_2))*180/pi;                                                           
            ...
            if( (xyz_11{1,fast_obj_start+2}(distance_thrd_mat(row_third(find_rest_loop),2),2)-yc_2) == 0 & (xyz_11{1,fast_obj_start+2}(distance_thrd_mat(row_third(find_rest_loop),2),1)-xc_2) ==0 )
        ...
        for find_rest_loop2 = 1: row_third_size(1,1) 
            if(stop_search2 == 0)
                ...
                diff_vector2=0;
                posit_nega2=nan;
                xc_4=xyz_11{1,fast_obj_start+2}(distance_thrd_mat(row_third(priority_angle_order3(find_rest_loop2,2)),2),1);
                yc_4=xyz_11{1,fast_obj_start+2}(distance_thrd_mat(row_third(priority_angle_order3(find_rest_loop2,2)),2),2);
