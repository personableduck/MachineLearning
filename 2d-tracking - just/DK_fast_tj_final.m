...
threshold_diff_dislocation_shrt2=1/(2); % 200%
control_vecter_angle = 23; %angle control
multilple_short_angle = 2.5; %find wiggling moving for slow one
...
% xyz_sub=xyz;
% 
% for subtract_loop_xyz=1:iter-2
% 
%     size_cmp_n=size(xyz_sub{1,subtract_loop_xyz+1});
%     
% for loop_cmp_del=1:size_cmp_n(1,1)
%      
%     [row_del_x]=find(xyz_sub{1,subtract_loop_xyz}(:,1) == xyz_sub{1,subtract_loop_xyz+1}(loop_cmp_del,1));
%     [row_del_y]=find(xyz_sub{1,subtract_loop_xyz}(:,2) == xyz_sub{1,subtract_loop_xyz+1}(loop_cmp_del,2));
%  
%     rr_ss=size(row_del_x);
%     for check_loop_dlt=1:rr_ss   
%         [row_del_f,col_del_f,check_del_f]=find(row_del_y == row_del_x(check_loop_dlt));        
...
%             check_delte_not=0;
%             
%             if(subtract_loop_xyz>1)
%                 [row_del_x2,col_del_f2,check_del_f2]=find(xyz_sub{1,subtract_loop_xyz-1}(:,1) == xyz_sub{1,subtract_loop_xyz+1}(loop_cmp_del,1));
%                 rr_ss2=size(row_del_x2);
%                 if(check_del_f2 ~= 0)
%                     for check_loop_dlt2=1:rr_ss2
%                     
%                         if( xyz_sub{1,subtract_loop_xyz-1}(row_del_x2(check_loop_dlt2),2) ==  xyz_sub{1,subtract_loop_xyz+1}(loop_cmp_del,2))
%                             check_delte_not=1;
%                         end
%                     end
%                 end
%             end
%             
%             if(check_delte_not == 0)
%             xyz_sub{1,subtract_loop_xyz}(row_del_x(check_loop_dlt),:)=nan;
%             end
%         
%         end
%     end 
...
for behinding_loop= 1: maxsize_behinding(1,1)
    ...
    if(check_strt == 1)
        ...
        for row_srt_dpl_loop=1:row_strt_size(1,1)
            ...
            if(check_dpl == 1)
                ...
                for row_strt_dpl_loop=1:row_strt_dpl_size(1,1);
                    xyz_1(researching_xyz1(row_srt_dpl(row_strt_dpl_loop)),:)=nan;
        ...
        if(count_skip_strt==0) 
            ...
            xyz_1(researching_xyz1(row_strt(order_dist_strt)),:)=nan;
        end      
    end
end
...
for rest_start_searching= 1:max_frame_size
    ...
    if(fast_obj_start == 1)
        maxsize_fast_xyz=size(xyz_1); %read size
    ...
    for inside_loop=1:change_inside_loop
        ...
        for fast_second_mt=1:maxsize_fast2_xyz(1,1) %for fast sperm
            ...
            if(fast_obj_start == 1)
                ...
                xx_1 = xyz{1,fast_obj_start+1}(fast_second_mt,1) - xyz_1(inside_loop,1); % comparing x coordinates   
                yy_1 = xyz{1,fast_obj_start+1}(fast_second_mt,2) - xyz_1(inside_loop,2); % comparing y coordinates
        ...
        if(fast_obj_start == 1) 
            ...
            if( cont_rest_th(inside_loop,1) <= decide_slow)
                [row_dist,col_dist,check_dist]=find(distance_fast_mat < decide_slow);            
            else
            ...
            for try_loop2 = 1: row_dist_size(1,1)
              ...
              if(stop_search == 0) 
                ...
                if(fast_obj_start == 1)
                    ...
                    %each magnitude
                    angle_fast = atan((xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),2)-xyz_1(inside_loop,2))/(xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1)-xyz_1(inside_loop,1)))*180/pi; % unit is degree
                    dist_thresh_sec = distance_fast_mat(row_dist(priority_angle_order(try_loop2,2)));
                
                    if(angle_fast <= 45 & angle_fast >= -45) %vetorc
                        vector=1; % x vetorc
                        diff_vector = xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1) - xyz_1(inside_loop,1);
                        posit_nega=sign(diff_vector); % x_vector>0 : 1 , x_vector<0 : -1 , 0=0
                    else
                        vector=0; % y vetorc
                        diff_vector = xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),2) - xyz_1(inside_loop,2); 
                        ...
                        if(pass_value == 0)       
                            if(pass_numfil == 1)
                                ...
                                if(dist_thresh_sec <= decide_slow)
                                    [row_secd,col_secd,check_secd]=find(distance_repeat_rest < decide_slow);
                                else
                                    ...
                                    for find_secd_loop2 = 1: row_secd_size(1,1)
                                      if(stop_search == 0)
                                        ...
                                        if(posit_nega == posit_nega2)
                                            ...
                                            if(fast_obj_start == 1)
                                                if((distance_repeat_rest(row_secd(priority_angle_order2(find_secd_loop2,2)))+dist_thresh_sec)*increase_distance_threshold < sqrt( (xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1)-xyz_1(inside_loop,1))^2+(xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2)-xyz_1(inside_loop,2))^2 ))
                                                    pass_value=0;
                                                else
                                                    pass_value=1;
                                                end
                                            ...
                                            if(pass_value == 0)                                       
                                                ...
                                                if(pass_numfil == 1) 
                                                    ...
                                                    if(dist_thresh_third <= decide_slow)
                                                        [row_third,col_third,check_third]=find(distance_thrd_mat < decide_slow);
                                                    else
                                                                    ...
                                                                    if(pass_value == 0)                                                      
                                                                        ...
                                                                        if( pass_numfil == 1)
                                                                            ...
                                                                            if(dist_rest_th <= decide_slow)     
                                                                                [row_four,col_four,check_four]=find(distance_four_mat < decide_slow);                              
                                                                            else
                                                                                            ...
                                                                                            if(pass_value == 0)                                                                                             
                                                                                                ...
                                                                                                if( pass_numfil == 1)
                                                                                                    ...
                                                                                                    if(fast_obj_start == 1)
                                                                                                        ...
                                                                                                        fst_mv_oj_coor(1,1) = xyz_1(inside_loop,1);                              
                                                                                                        fst_mv_oj_coor(1,2) = xyz_1(inside_loop,2);          
                                                                                                        ...
                                                                                                        angle_information(object_number_count,1)=abs(abs(angle_fast-angle_secd_fast)+abs(angle_secd_fast-angle_rest_th)+abs(angle_rest_th-angle_final_th));
