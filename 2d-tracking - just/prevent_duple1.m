function [fast_total_coor,new_search1,stop_search,object_number_count]=prevent_duple1(fst_mv_oj_coor,fast_total_coor,object_number_count,stop_search,angle_information,fast_obj_start,xyz_1)
...
if( fast_total_coor{1,1} == 0)
    ...
    if(duplication_number ~= 0)                                                                                  
        ...
        if( duplehappen == 1)
            ...
            [row_srh_x,col_srh_x,check_srh_x]=find(xyz_1{1,fast_obj_start}(:,1) == fast_total_coor{1,duplication_number}(fast_obj_start,1));
            [row_srh_y,col_srh_y,check_srh_y]=find(xyz_1{1,fast_obj_start}(:,2) == fast_total_coor{1,duplication_number}(fast_obj_start,2));
