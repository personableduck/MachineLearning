...
for rest_start_searching= 1:max_frame_size
    ...
    for inside_loop=1:change_inside_loop
        ...
        if(check_f1 == 1)
            ...
            if(fast_obj_start == 1) 
                [row_dist,col_dist,check_dist]=find(distance_fast_mat <= distance_threshold_fast & distance_fast_mat ~= 0);     
            else
                if( cont_rest_th(inside_loop,1) <= decide_slow)
                    [row_dist,col_dist,check_dist]=find(distance_fast_mat <= distance_threshold_fast & distance_fast_mat ~= 0);            
                else
                    [row_dist,col_dist,check_dist]=find(distance_fast_mat <= cont_rest_th(inside_loop,1)*threshold_diff_dislocation & distance_fast_mat ~= 0);
                end
            end
                                ...
                                if(pass_numfil == 1) 
                                    ...
                                    if(check_f2 == 1)
                                        ...
                                        if(dist_thresh_sec <= decide_slow)
                                            [row_secd,col_secd,check_secd]=find(distance_repeat_rest < distance_threshold_fast & distance_repeat_rest ~= 0);
                                        else
                                            [row_secd,col_secd,check_secd]=find(distance_repeat_rest < dist_thresh_sec*threshold_diff_dislocation & distance_repeat_rest <= distance_threshold_fast & distance_repeat_rest ~= 0);
                                        end
                                        if (check_secd == 1)
                                                            ...
                                                            if(pass_numfil == 1) 
                                                                ...
                                                                if(check_f3 == 1)
                                                                    ...
                                                                    if(dist_thresh_third <= decide_slow)
                                                                        [row_third,col_third,check_third]=find(distance_thrd_mat < distance_threshold_fast & distance_thrd_mat ~= 0);
                                                                    else
                                                                        [row_third,col_third,check_third]=find(distance_thrd_mat < dist_thresh_third*threshold_diff_dislocation & distance_thrd_mat <= distance_threshold_fast & distance_thrd_mat ~= 0);
                                                                    end
                                                                    if (check_third == 1)
                                                                                        ...
                                                                                        if( pass_numfil == 1) 
                                                                                            ...
                                                                                            if(check_f4 == 1)
                                                                                                ...
                                                                                                if(dist_rest_th <= decide_slow)     
                                                                                                    [row_four,col_four,check_four]=find(distance_four_mat < distance_threshold_fast & distance_four_mat ~= 0);                              
                                                                                                else
                                                                                                    [row_four,col_four,check_four]=find(distance_four_mat < dist_rest_th*threshold_diff_dislocation & distance_four_mat <= distance_threshold_fast & distance_four_mat ~= 0);   
                                                                                                end
                                                                                                if (check_four == 1)
                                                                                                                        ...
                                                                                                                        if(fast_obj_start == 1)
                                                                                                                            ...
                                                                                                                            if(check_f5 == 1)
                                                                                                                                ...
                                                                                                                                if( dist_final_th > decide_slow)
                                                                                                                                    ...
                                                                                                                                    if(dist_final_th <= decide_slow)
                                                                                                                                        [row_six,col_six,check_six]=find(distance_six_mat < distance_threshold_fast & distance_six_mat ~= 0);
                                                                                                                                    else
                                                                                                                                        [row_six,col_six,check_six]=find(distance_six_mat < dist_final_th*threshold_diff_dislocation & distance_six_mat <= distance_threshold_fast & distance_six_mat ~= 0);
                                                                                                                                    end
                                                                                                                                            ...
                                                                                                                                            if(stop_search2 == 0)
                                                                                                                                                ...
                                                                                                                                                if(posit_nega == posit_nega2)
                                                                                                                                                    ...
                                                                                                                                                    if( increase_check == 0)
                                                                                                                                                        ...
                                                                                                                                                        if( abs(angle_secd_fast) > tangent_control)
                                                                                                                                                            if( abs(abs(angle_rest_th)-abs(angle_final_th)) + control_vecter_angle * compare_angle_time > abs(abs(angle_final_th)-abs(angle_six_th_test)) & abs(abs(angle_rest_th)-abs(angle_final_th)) - control_vecter_angle * compare_angle_time < abs(abs(angle_final_th)-abs(angle_six_th_test)) )
                                                                                                                                                                pass_value=0;
                                                                                                                                                            else
                                                                                                                                                                pass_value=1;  
                                                                                                                                                            end
                                                                                                                                                        else  
                                                                                                                                                            if( abs(angle_rest_th-angle_final_th) + control_vecter_angle * compare_angle_time > abs(angle_final_th-angle_six_th_test) & abs(angle_rest_th-angle_final_th) + control_vecter_angle * compare_angle_time > abs(angle_final_th-angle_six_th_test) )
                                                                                                                                                                pass_value=0;
                                                                                                                                                            else
                                                                                                                                                                pass_value=1;  
                                                                                                                                                            end
