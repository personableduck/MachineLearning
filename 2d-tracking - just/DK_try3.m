            ...
            if (min_intensities_xyz{1,fast_obj_start}(inside_loop) > min_intensities_xyz{1,fast_obj_start}(inside_loop+row_dist(try_loop))) % compare intensity to decide before one. smaller intensity means later one       
                ...
                if (check_secd == 1)
                    ...
                    for find_secd_loop = 1: row_secd_size(1,1)
                        ...
                        if(abs(angle_fast)+23 > abs(atan((xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),2)-xyz{1,fast_obj_start}((inside_loop+row_dist(try_loop)),2))/((xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),1))-xyz{1,fast_obj_start}((inside_loop+row_dist(try_loop)),1)))*180/pi) & abs(angle_fast)-23 < abs(atan((xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),2)-xyz{1,fast_obj_start}((inside_loop+row_dist(try_loop)),2))/((xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),1))-xyz{1,fast_obj_start}((inside_loop+row_dist(try_loop)),1)))*180/pi)) % unit is degree
                            ...
                            [row_rest,col_rest,check_rest]=find(distance_thrd_mat <= dist_thresh_third * (1.3) & distance_thrd_mat >= dist_thresh_third * (0.7));
                
                            if (check_rest == 1)
                               
                                row_third_size=size(row_rest);
                                angle_rest_th=0;
                                dist_rest_th=0;
                                             
                                for find_rest_loop = 1: row_third_size(1,1)

                                    if(abs(angle_secd_fast)+23 > abs(atan((xyz{1,fast_obj_start+2}(row_rest(find_rest_loop),2)-(xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),2)))/(xyz{1,fast_obj_start+2}(row_rest(find_rest_loop),1)-(xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),1))))*180/pi) & abs(angle_secd_fast)-23 < abs(atan((xyz{1,fast_obj_start+2}(row_rest(find_rest_loop),2)-(xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),2)))/(xyz{1,fast_obj_start+2}(row_rest(find_rest_loop),1)-(xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),1))))*180/pi))     
   
                                        angle_rest_th=atan((xyz{1,fast_obj_start+2}(row_rest(find_rest_loop),2)-(xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),2)))/(xyz{1,fast_obj_start+2}(row_rest(find_rest_loop),1)-(xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),1))))*180/pi;
                                        dist_rest_th=distance_thrd_mat(row_rest(find_rest_loop));
...
%     for rest_cmp_loop = 1: loop_dist_size(1,1)
%         
%        xx_rest = xyz{1,rest_trajectory}(rest_cmp_loop,1) - xyz{1,rest_trajectory-1}(row_rest(find_rest_loop),1); %need to fix find the value for loop (xyz{1,fast_obj_start+2}(row_third(find_third_loop),1))
%        yy_rest = xyz{1,rest_trajectory}(rest_cmp_loop,2) - xyz{1,rest_trajectory-1}(row_rest(find_rest_loop),2);
%                 
%        distance_rest_mat(rest_cmp_loop,1) = sqrt((xx_rest^2)+(yy_rest^2));
%         
%     end
%     
%     [row_rest,col_rest,check_rest]=find(distance_rest_mat <= dist_thresh_four * (1.3) & distance_thrd_mat >= dist_thresh_four * (0.7)); %find (dist_thresh_four)
%                               
%     if (check_rest == 1)                       
%         
%         row_rest_size=size(row_rest);
%         
%         for find_rest_loop = 1: row_rest_size(1,1)
%             
%             if(abs(angle_third_fast)+45 > abs(atan((xyz{1,rest_trajectory}(row_rest(find_rest_loop),2)-xyz{1,fast_obj_start+2}(row_rest(find_rest_loop),2)) / (xyz{1,rest_trajectory}(row_rest(find_rest_loop),1)-xyz{1,fast_obj_start+2}(row_rest(find_rest_loop),1)))*180/pi) & abs(angle_third_fast)-45 < abs(atan((xyz{1,rest_trajectory}(row_rest(find_rest_loop),2)-xyz{1,fast_obj_start+2}(row_rest(find_rest_loop),2)) / (xyz{1,rest_trajectory}(row_rest(find_rest_loop),1)-xyz{1,fast_obj_start+2}(row_rest(find_rest_loop),1)))*180/pi));     %find angle_third_fast
%                     %*******************progress here check******************  
%                     angle_rest_fast= atan((xyz{1,rest_trajectory}(row_rest(find_rest_loop),2)-xyz{1,fast_obj_start+2}(row_rest(find_rest_loop),2)) / (xyz{1,rest_trajectory}(row_rest(find_rest_loop),1)-xyz{1,fast_obj_start+2}(row_rest(find_rest_loop),1)))*180/pi;
%                     dist_rest_four=distance_rest_mat(row_rest(find_rest_loop));
%                     
%                     fst_mv_oj_coor(rest_trajectory+1,1)=xyz{1,rest_trajectory}(row_rest(find_rest_loop),1); %x
%                     fst_mv_oj_coor(rest_trajectory+1,2)=xyz{1,rest_trajectory}(row_rest(find_rest_loop),2); %y
%                     fst_mv_oj_coor(rest_trajectory+1,3)=2; %speed distinguish
%                     fst_mv_oj_coor(rest_trajectory+1,4)=object_number_count; %object number
%                     
%                     fast_total_coor(1,object_number_count)={fst_mv_oj_coor}; %need to consider
