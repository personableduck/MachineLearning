                            ...
                            if (check_third == 1)
                                ...
                                for find_rest_loop = 1: row_third_size(1,1)
                                      ...
                                      if(angle_secd_fast+control_vecter_angle > atan((xyz{1,fast_obj_start+2}(row_third(find_rest_loop),2)-(xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),2)))/(xyz{1,fast_obj_start+2}(row_third(find_rest_loop),1)-(xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),1))))*180/pi & angle_secd_fast-control_vecter_angle < atan((xyz{1,fast_obj_start+2}(row_third(find_rest_loop),2)-(xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),2)))/(xyz{1,fast_obj_start+2}(row_third(find_rest_loop),1)-(xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),1))))*180/pi)     
                                        ...
                                        fst_mv_oj_coor(4,1) = xyz{1,fast_obj_start+2}(row_third(find_rest_loop),1);
                                        fst_mv_oj_coor(4,2) = xyz{1,fast_obj_start+2}(row_third(find_rest_loop),2);          
                                        ...
                                        cont_rest_th(1,object_number_count)=dist_rest_th;
                                        cont_angle_th(1,object_number_count)=angle_rest_th;
...
%----find rest trajectories


max_frame_size=fix((iter-1)/3)-1;

for bundle_frame=1:max_frame_size

bundle_frame=1;

fast_obj_start=3*bundle_frame+1; % if you need to find starting point on the middle of frame, then you need to change the value as a loop(for function)
maxsize_fast_xyz=size(xyz{1,fast_obj_start}); %read size  
...
for rest_tj_loop=1:object_number_count
    ...
    distance_fast_mat=0;
        ...
        xx_1 = xyz{1,fast_obj_start}(inside_loop,1) - fast_total_coor{1,rest_tj_loop}(fast_obj_start,1); % comparing x coordinates   
        yy_1 = xyz{1,fast_obj_start}(inside_loop,2) - fast_total_coor{1,rest_tj_loop}(fast_obj_start,2); % comparing y coordinates 
        
        distance_fast_mat(inside_loop,1) = sqrt((xx_1^2)+(yy_1^2));
        end

    [row_dist,col_dist,check_dist]=find(distance_fast_mat <= cont_rest_th(1,rest_tj_loop));
    ...
    if (check_dist == 1) %first distance threshold check
        ...
        for try_loop = 1: row_dist_size(1,1)
            if(cont_rest_th(1,rest_tj_loop)*1.3 < sqrt( (xyz{1,fast_obj_start}(row_dist(try_loop),1)-fast_total_coor{1,rest_tj_loop}(fast_obj_start-1,1))^2+(xyz{1,fast_obj_start}(row_dist(try_loop),2)-fast_total_coor{1,rest_tj_loop}(fast_obj_start-1,2))^2 ))
            if(cont_angle_th(1,rest_tj_loop)+control_vecter_angle > atan((xyz{1,fast_obj_start}(row_dist(try_loop),2)-fast_total_coor{1,rest_tj_loop}(fast_obj_start,2))/(xyz{1,fast_obj_start}(row_dist(try_loop),1)-fast_total_coor{1,rest_tj_loop}(fast_obj_start,1)))*180/pi & cont_angle_th(1,rest_tj_loop)-control_vecter_angle < atan((xyz{1,fast_obj_start}(row_dist(try_loop),2)-fast_total_coor{1,rest_tj_loop}(fast_obj_start,2))/(xyz{1,fast_obj_start}(row_dist(try_loop),1)-fast_total_coor{1,rest_tj_loop}(fast_obj_start,1)))*180/pi)

            
                %decide vector andgle
                %each magnitude
                angle_fast = (atan((xyz{1,fast_obj_start}(row_dist(try_loop),2)-fast_total_coor{1,rest_tj_loop}(fast_obj_start,2))/(xyz{1,fast_obj_start}(row_dist(try_loop),1)-fast_total_coor{1,rest_tj_loop}(fast_obj_start,1))))*180/pi; % unit is degree
                dist_thresh_sec = distance_fast_mat(row_dist(try_loop));
                ...
                for dist_cmp_loop = 1: sec_dist_size(1,1)
                    ...
                    xx_2 = xyz{1,fast_obj_start+1}(dist_cmp_loop,1) - xyz{1,fast_obj_start}(row_dist(try_loop),1);
                    yy_2 = xyz{1,fast_obj_start+1}(dist_cmp_loop,2) - xyz{1,fast_obj_start}(row_dist(try_loop),2);
                ...
                if (check_secd == 1)
                    ...
                    for find_secd_loop = 1: row_secd_size(1,1)
                        if(dist_thresh_sec*1.3 < sqrt( (xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),1)-fast_total_coor{1,rest_tj_loop}(fast_obj_start,1))^2+(xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),2)-fast_total_coor{1,rest_tj_loop}(fast_obj_start,2))^2 ))
                        if(angle_fast+control_vecter_angle > atan((xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),2)-xyz{1,fast_obj_start}(row_dist(try_loop),2))/(xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),1))-xyz{1,fast_obj_start}((row_dist(try_loop)),1))*180/pi & angle_fast-control_vecter_angle < atan((xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),2)-xyz{1,fast_obj_start}((row_dist(try_loop)),2))/((xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),1))-xyz{1,fast_obj_start}((row_dist(try_loop)),1)))*180/pi) % unit is degree
             
                            angle_secd_fast = atan((xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),2)-xyz{1,fast_obj_start}(row_dist(try_loop),2))/(xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),1)-xyz{1,fast_obj_start}(row_dist(try_loop),1)))*180/pi;
                            dist_thresh_third = distance_secd_mat(row_secd(find_secd_loop));
                            % finish third check
                            % (xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),1)),(xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),2))
                            ...
                            if (check_third == 1)
                                ...
                                for find_rest_loop = 1: row_third_size(1,1)
                                    ...
                                    if(dist_thresh_third*1.3 < sqrt((xyz{1,fast_obj_start+2}(row_third(find_rest_loop),1)- xyz{1,fast_obj_start}(row_dist(try_loop),1))^2+(xyz{1,fast_obj_start+2}(row_third(find_rest_loop),2)- xyz{1,fast_obj_start}(row_dist(try_loop),2))^2))
                                    if(angle_secd_fast+control_vecter_angle > atan((xyz{1,fast_obj_start+2}(row_third(find_rest_loop),2)-xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),2))/(xyz{1,fast_obj_start+2}(row_third(find_rest_loop),1)-xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),1)))*180/pi & angle_secd_fast-control_vecter_angle < atan((xyz{1,fast_obj_start+2}(row_third(find_rest_loop),2)-(xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),2)))/(xyz{1,fast_obj_start+2}(row_third(find_rest_loop),1)-(xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),1))))*180/pi)     
                                        ...
                                        fast_total_coor{1,rest_tj_loop}(fast_obj_start+1,1) = xyz{1,fast_obj_start}(row_dist(try_loop),1);
                                        fast_total_coor{1,rest_tj_loop}(fast_obj_start+1,2) = xyz{1,fast_obj_start}(row_dist(try_loop),2);          
                                        fast_total_coor{1,rest_tj_loop}(fast_obj_start+1,3) = 2; %fast moving value is 2
                                        fast_total_coor{1,rest_tj_loop}(fast_obj_start+1,4) = rest_tj_loop;
                                   
                                        fast_total_coor{1,rest_tj_loop}(fast_obj_start+2,1) = xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),1);
                                        fast_total_coor{1,rest_tj_loop}(fast_obj_start+2,2) = xyz{1,fast_obj_start+1}(row_secd(find_secd_loop),2);          
                                        fast_total_coor{1,rest_tj_loop}(fast_obj_start+2,3) = 2; %fast moving value is 2
                                        fast_total_coor{1,rest_tj_loop}(fast_obj_start+2,4) = rest_tj_loop;
                                        
                                        fast_total_coor{1,rest_tj_loop}(fast_obj_start+3,1) = xyz{1,fast_obj_start+2}(row_third(find_rest_loop),1);
                                        fast_total_coor{1,rest_tj_loop}(fast_obj_start+3,2) = xyz{1,fast_obj_start+2}(row_third(find_rest_loop),2);          
                                        fast_total_coor{1,rest_tj_loop}(fast_obj_start+3,3) = 2; %fast moving value is 2
                                        fast_total_coor{1,rest_tj_loop}(fast_obj_start+3,4) = rest_tj_loop;
                                        
                                        
                                        cont_rest_th(1,rest_tj_loop)=dist_rest_th;
                                        cont_angle_th(1,object_number_count)=angle_rest_th;
...
size_xyz1=size(xyz{1,1});

count_siced=0;

for loop_rearrange=1:size_xyz1(1,1)
    [a,aa,check_a]=find(xyz{1,2}(:,1) == xyz{1,1}(loop_rearrange,1));
    [b,bb,check_b]=find(xyz{1,2}(:,2) == xyz{1,1}(loop_rearrange,2));
        if(check_a==1 & check_b==1)
            [c]=find(a==b);
            count_siced=count_siced+1;
            
            a_cmp=size(a);
            b_cmp=size(b); 
            if(a_cmp(1,1) >= b_cmp(1,1))
                sliced_xyz(count_siced,:)=xyz{1,2}(a(c),:); %(c)value is two
            else           
                sliced_xyz(count_siced,:)=xyz{1,2}(b(c),:); 
            end         
        end    
end
