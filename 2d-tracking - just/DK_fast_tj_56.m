                        ...
                        if(fast_obj_start == 1)                         
                            ...
                            if(increase_check == 0)                
                                if(abs(cont_angle_th(inside_loop,1)) > tangent_control)
                                    if( abs(cont_angle_cmp(inside_loop,1)) + control_vecter_angle * compare_angle_time > abs(abs(cont_angle_th(inside_loop,1)) - abs(angle_fast_test)) & abs(cont_angle_cmp(inside_loop,1)) - control_vecter_angle * compare_angle_time < abs(abs(cont_angle_th(inside_loop,1)) - abs(angle_fast_test)) )
                                        pass_value=0; 
                                    else
                                        pass_value=1;    
                                    end
                                else
                                    if( abs(cont_angle_cmp(inside_loop,1)) + control_vecter_angle * compare_angle_time > abs(cont_angle_th(inside_loop,1) - angle_fast_test) & abs(cont_angle_cmp(inside_loop,1)) - control_vecter_angle * compare_angle_time < abs(cont_angle_th(inside_loop,1) - angle_fast_test) )
                                        pass_value=0; 
                                    else
                                        pass_value=1;    
                                    end
                                                ...
                                                if(fast_obj_start == 1)
                                                    ...
                                                    if(increase_check == 0)
                                                        ...
                                                        if(abs(angle_fast) > tangent_control)
                                                            if( abs(abs(cont_angle_th(inside_loop,1))-abs(angle_fast)) + control_vecter_angle * compare_angle_time > abs(abs(angle_fast) - abs(angle_secd_fast_test)) & abs(abs(cont_angle_th(inside_loop,1))-abs(angle_fast)) - control_vecter_angle * compare_angle_time < abs(abs(angle_fast) - abs(angle_secd_fast_test)) )
                                                                pass_value=0;
                                                            else
                                                                pass_value=1;
                                                            end 
                                                        else
                                                            if( abs(cont_angle_th(inside_loop,1)-angle_fast) + control_vecter_angle * compare_angle_time > abs(angle_fast - angle_secd_fast_test) & abs(cont_angle_th(inside_loop,1)-angle_fast) - control_vecter_angle * compare_angle_time < abs(angle_fast - angle_secd_fast_test) )
                                                                pass_value=0;
                                                            else
                                                                pass_value=1;
                                                            end    
                                                                    ...
                                                                    if(posit_nega == posit_nega2)
                                                                        ...
                                                                        if( increase_check == 0)
                                                                            ...
                                                                            if( abs(angle_secd_fast) > tangent_control)
                                                                                if( abs(abs(angle_fast)-abs(angle_secd_fast)) + control_vecter_angle * compare_angle_time > abs(abs(angle_secd_fast)-abs(angle_rest_th_test)) & abs(abs(angle_fast)-abs(angle_secd_fast)) + control_vecter_angle * compare_angle_time > abs(abs(angle_secd_fast)-abs(angle_rest_th_test)) )
                                                                                    pass_value=0;
                                                                                else
                                                                                    pass_value=1;  
                                                                                end
                                                                            else  
                                                                                if( abs(angle_fast-angle_secd_fast) + control_vecter_angle * compare_angle_time > abs(angle_secd_fast-angle_rest_th_test) & abs(angle_fast-angle_secd_fast) + control_vecter_angle * compare_angle_time > abs(angle_secd_fast-angle_rest_th_test) )
                                                                                    pass_value=0;
                                                                                else
                                                                                    pass_value=1;  
                                                                                end
                                                                                            ...
                                                                                            if(posit_nega == posit_nega2)
                                                                                                ...
                                                                                                if( increase_check == 0)                                                                                      
                                                                                                    ...
                                                                                                    if(abs(angle_rest_th) > tangent_control)
                                                                                                        if( abs(abs(angle_secd_fast)-abs(angle_rest_th)) + control_vecter_angle * compare_angle_time > abs(abs(angle_rest_th)-abs(angle_final_th_test)) & abs(abs(angle_secd_fast)-abs(angle_rest_th)) - control_vecter_angle * compare_angle_time < abs(abs(angle_rest_th)-abs(angle_final_th_test)) )
                                                                                                            pass_value=0;    
                                                                                                        else
                                                                                                            pass_value=1;      
                                                                                                        end
                                                                                                    else 
                                                                                                        if( abs(angle_secd_fast-angle_rest_th) + control_vecter_angle * compare_angle_time > abs(angle_rest_th-angle_final_th_test) & abs(angle_secd_fast-angle_rest_th) - control_vecter_angle * compare_angle_time < abs(angle_rest_th-angle_final_th_test) )
                                                                                                            pass_value=0;    
                                                                                                        else
                                                                                                            pass_value=1;      
                                                                                                        end
