function [fast_total_coor,prevent_rest_check,duple_mat,stop_search]=slow_duple2(fst_mv_oj_coor,fast_total_coor,inside_loop,prevent_rest_check,ans_xyz3,origin_size_fst,jump_number,object_number_count,fast_obj_start)
%-----prevent duplication----
    ...
    for prevent_duplication_loop=object_number_count+1:maxsize_fstc(1,2)	
        ...
        for fst_check_loop =1:maxsize_cmp_fst(1,1)
            ...
            for dplc_chk_loop = 1:rr_dplc
                [~,col_find_dplc,check_find_dplc]= find(row_dplc_y == row_dplc_x(dplc_chk_loop));
...
if(duplication_number ~= 0)
    ...
    if( duplehappen == 1)
        ...
        fast_total_coor{1,inside_loop}(origin_size_fst+1,4) = fst_mv_oj_coor(1,4); 

        fast_total_coor{1,inside_loop}(origin_size_fst+2,1) = fst_mv_oj_coor(2,1);
        fast_total_coor{1,inside_loop}(origin_size_fst+2,2) = fst_mv_oj_coor(2,2); 
        fast_total_coor{1,inside_loop}(origin_size_fst+2,3) = fst_mv_oj_coor(2,3);
        fast_total_coor{1,inside_loop}(origin_size_fst+2,4) = fst_mv_oj_coor(2,4);
        ...
        if(~isnan(ans_xyz3))
            ...
            fast_total_coor{1,inside_loop}(origin_size_fst+3,4) = fst_mv_oj_coor(3,4);
        end
