...
%------solve diff holo dislocation
threshold_diff_dislocation= 1.3; %check it!! important value


for combine_compare_loop=1:iter-2
    
    front_compare=combine_compare_loop*2;
    behind_compare=front_compare+1;
    
    maxsize_cmb_cp=size(devided_coordinates{1,front_compare}); %read size
    input_cmb_cp=maxsize_cmb_cp(1,1); 
    
    maxsize_cmb_cp2=size(devided_coordinates{1,behind_compare}); %read size
    input_cmb_cp2=maxsize_cmb_cp2(1,1);
    
    for inft_loop=1:input_cmb_cp
        for bhid_loop=1:input_cmb_cp2

            xx_shrt = devided_coordinates{1,front_compare}(inft_loop,1) - devided_coordinates{1,behind_compare}(bhid_loop,1); % comparing x coordinates
            yy_shrt = devided_coordinates{1,front_compare}(inft_loop,2) - devided_coordinates{1,behind_compare}(bhid_loop,2); % comparing y coordinates
        
            distance_shrt_limit= sqrt((xx_shrt^2)+(yy_shrt^2));

            if (distance_shrt_limit <= threshold_diff_dislocation)         
                devided_coordinates{1,front_compare}(inft_loop,1) = devided_coordinates{1,behind_compare}(bhid_loop,1);
                devided_coordinates{1,front_compare}(inft_loop,2) = devided_coordinates{1,behind_compare}(bhid_loop,2);
            end
        end
    end
...
dkcor= size(object_trajectories);
dkcormax= dkcor(1,2);
...
spd_loopnum= size(object_trajectories{1,ababs});
size_spd_obj= spd_loopnum(1,1);
...
for objds = 1:size_spd_obj-1
    
    obdistances= sqrt((object_trajectories{1,ababs}(objds+1,1)-object_trajectories{1,ababs}(objds,1))^2 + (object_trajectories{1,ababs}(objds+1,2)-object_trajectories{1,ababs}(objds,2))^2); % um
