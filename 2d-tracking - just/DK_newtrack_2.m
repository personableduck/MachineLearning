...
control_vecter_angle = 45; %angle control
multilple_short_angle = 1.78; %find wiggling moving for slow one
increase_distance_threshold=0.75; %make sure about linear moving for fast
        ...
        if(check_f1 == 1)
            ...
            for fast_second_mt=1:maxsize_fast2_xyz(1,1) %for fast sperm
                if(fast_obj_start == 1)
                    ...
                    distance_fast_mat(row_f1(fast_second_mt),1) = sqrt((xx_1^2)+(yy_1^2));
                else
                    estm_ft_size=size(fast_total_coor{1,inside_loop});
                    if( estm_ft_size(1,1) >= fast_obj_start )
                    ...
                    distance_fast_mat(row_f1(fast_second_mt),1) = sqrt((xx_1^2)+(yy_1^2));
                    if( xx_1 == 0 & yy_1 == 0)
                        distance_fast_mat(row_f1(fast_second_mt),1) = 0.1;
                    end
                end
            end
            [row_d1]=find(distance_fast_mat==0);
            size_row_d1=size(row_d1)
            for index=1:size_row_d1(1,2)
                distance_fast_mat(row_d1(index),1)=nan;
            end
            [row_back1]=find(distance_fast_mat==0.1);
            size_row_back1=size(row_back1)
            for index=1:size_row_back1(1,2)
                distance_fast_mat(row_back1(index),1)=0;
            end
            if(fast_obj_start == 1) 
                ...
                if( cont_rest_th(inside_loop,1) <= decide_slow)
                    ...
                    [row_dist,col_dist,check_dist]=find(distance_fast_mat <= cont_rest_th(inside_loop,1)*threshold_diff_dislocation);
                end
            end
            if (check_dist == 1) %first distance threshold check
                ...
                if(fast_obj_start == 1)  
                    ...
                    for try_loop = 1: row_dist_size(1,1) 
                        ...
                        if( (xyz{1,fast_obj_start+1}(row_dist(try_loop),2)-fast_total_coor{1,inside_loop}(fast_obj_start,2)) == 0 & (xyz{1,fast_obj_start+1}(row_dist(try_loop),1)-fast_total_coor{1,inside_loop}(fast_obj_start,1)) == 0 )
                            angle_fast_test = 0;
                        end
                ...
                for try_loop2 = 1: row_dist_size(1,1)
                    if(stop_search == 0) 
                        ...
                        if(fast_obj_start == 1)
                            ...
                            if( (xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),2)-xyz{1,fast_obj_start}(inside_loop,2)) == 0 & (xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1)-xyz{1,fast_obj_start}(inside_loop,1)) == 0 )
                                angle_fast = 0;
                            end
                            ...
                            if( (xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),2)-fast_total_coor{1,inside_loop}(fast_obj_start,2)) == 0 & (xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1)-fast_total_coor{1,inside_loop}(fast_obj_start,1)) == 0)
                                angle_fast_test = 0;
                            end
                            if( cont_rest_th(inside_loop,1) > decide_slow_angle)           
                                if(abs(cont_angle_th(inside_loop,1)) > tangent_control | abs(angle_fast_test) > tangent_control)              
                                    if(abs(cont_angle_th(inside_loop,1))+control_vecter_angle > abs(atan((xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),2)-fast_total_coor{1,inside_loop}(fast_obj_start-1,2))/(xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1)-fast_total_coor{1,inside_loop}(fast_obj_start-1,1)))*180/pi) & abs(cont_angle_th(inside_loop,1))-control_vecter_angle < abs(atan((xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),2)-fast_total_coor{1,inside_loop}(fast_obj_start-1,2))/(xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1)-fast_total_coor{1,inside_loop}(fast_obj_start-1,1)))*180/pi)) % unit is degree
                                        pass_numfil=1;
                                    end
                                else
                                    if(cont_angle_th(inside_loop,1)+control_vecter_angle > atan((xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),2)-fast_total_coor{1,inside_loop}(fast_obj_start-1,2))/(xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1)-fast_total_coor{1,inside_loop}(fast_obj_start-1,1)))*180/pi & cont_angle_th(inside_loop,1)-control_vecter_angle < atan((xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),2)-fast_total_coor{1,inside_loop}(fast_obj_start-1,2))/(xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1)-fast_total_coor{1,inside_loop}(fast_obj_start-1,1)))*180/pi) % unit is degree
                                    pass_numfil=1;
                                    end
                                end    
                            else
                                if(abs(cont_angle_th(inside_loop,1)) > tangent_control | abs(angle_fast_test) > tangent_control)              
                                    if(abs(cont_angle_th(inside_loop,1))+control_vecter_angle*multilple_short_angle > abs(atan((xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),2)-fast_total_coor{1,inside_loop}(fast_obj_start-1,2))/(xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1)-fast_total_coor{1,inside_loop}(fast_obj_start-1,1)))*180/pi) & abs(cont_angle_th(inside_loop,1))-control_vecter_angle*multilple_short_angle < abs(atan((xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),2)-fast_total_coor{1,inside_loop}(fast_obj_start-1,2))/(xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1)-fast_total_coor{1,inside_loop}(fast_obj_start-1,1)))*180/pi)) % unit is degree
                                        pass_numfil=1;
                                    end
                                else
                                    if(cont_angle_th(inside_loop,1)+control_vecter_angle*multilple_short_angle > atan((xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),2)-fast_total_coor{1,inside_loop}(fast_obj_start-1,2))/(xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1)-fast_total_coor{1,inside_loop}(fast_obj_start-1,1)))*180/pi & cont_angle_th(inside_loop,1)-control_vecter_angle*multilple_short_angle < atan((xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),2)-fast_total_coor{1,inside_loop}(fast_obj_start-1,2))/(xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1)-fast_total_coor{1,inside_loop}(fast_obj_start-1,1)))*180/pi) % unit is degree
                                    pass_numfil=1;
                                    end
                                end
                            end
                        ...
                        if(posit_nega == posit_nega2) 
                            ...
                            if(increase_check == 0)       
                                if(pass_numfil == 1) 
                                    ...
                                    [row_f2,col_f2,check_f2]=find( xyz{1,fast_obj_start+2}(:,1) < xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1) + distance_threshold_fast & xyz{1,fast_obj_start+2}(:,1) > xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1) - distance_threshold_fast);       
                                    if(check_f2 == 1)
                                        maxsize_fast_xyz2=size(row_f2); %read size 
                                        for dist_cmp_loop = 1: maxsize_fast_xyz2(1,1)
                                            xx_2 = xyz{1,fast_obj_start+2}(row_f2(dist_cmp_loop),1) - xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1);                              
                                            yy_2 = xyz{1,fast_obj_start+2}(row_f2(dist_cmp_loop),2) - xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),2);
                                            distance_repeat_rest(row_f2(dist_cmp_loop),1) = sqrt((xx_2^2)+(yy_2^2)); 
                                            if( xx_2 == 0 & yy_2 == 0)
                                                distance_repeat_rest(row_f2(dist_cmp_loop),1) = 0.1;
                                            end
                                        end 
                                        [row_d2]=find(distance_repeat_rest==0);
                                        size_row_d2=size(row_d2)
                                        for index=1:size_row_d2(1,2)
                                            distance_repeat_rest(row_d2(index),1)=nan;
                                        end
                                        [row_back2]=find(distance_repeat_rest==0.1);
                                        size_row_back2=size(row_back2)
                                        for index=1:size_row_back2(1,2)
                                            distance_repeat_rest(row_back2(index),1)=0;
                                        end
                                        ...
                                        if (check_secd == 1)
                                            ...
                                            for find_secd_loop = 1: row_secd_size(1,1)
                                                ...
                                                if( (xyz{1,fast_obj_start+2}(row_secd(find_secd_loop),2)-xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),2)) == 0 & (xyz{1,fast_obj_start+2}(row_secd(find_secd_loop),1)-xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1)) == 0 )
                                                    angle_secd_fast_test=0;
                                                end
                                                ...
                                                if(priority_angle_order2(find_secd_loop,1) == 0)
                                                ...
                                                if(stop_search == 0)
                                                    ...
                                                    if(posit_nega == posit_nega2)
                                                        ...
                                                        if(increase_check == 0)
                                                            ...
                                                            if( (xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2)-xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),2)) == 0 & (xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1)-xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1)) == 0 )
                                                            ...
                                                            if(fast_obj_start==1)
                                                                if(dist_thresh_sec > decide_slow_angle)
                                                                    if(abs(angle_fast)>tangent_control | abs(angle_secd_fast_cmp) > tangent_control)                                                     
                                                                        if(abs(angle_fast)+control_vecter_angle > abs(atan((xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2)-xyz{1,fast_obj_start}(inside_loop,2) )/(xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1)-xyz{1,fast_obj_start}(inside_loop,1)))*180/pi) & abs(angle_fast)-control_vecter_angle < abs(atan((xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2)-xyz{1,fast_obj_start}(inside_loop,2) )/(xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1)-xyz{1,fast_obj_start}(inside_loop,1)))*180/pi)) % unit is degree                       
                                                                            pass_numfil=1;
                                                                        end
                                                                    else
                                                                        if(angle_fast+control_vecter_angle > atan((xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2)-xyz{1,fast_obj_start}(inside_loop,2) )/(xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1)-xyz{1,fast_obj_start}(inside_loop,1)))*180/pi & angle_fast-control_vecter_angle < atan((xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2)-xyz{1,fast_obj_start}(inside_loop,2) )/(xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1)-xyz{1,fast_obj_start}(inside_loop,1)))*180/pi) % unit is degree  
                                                                            pass_numfil=1;
                                                                        end
                                                                    end
                                                                else
                                                                ...
                                                                if(dist_thresh_sec > decide_slow_angle)
                                                                    if(abs(angle_fast)>tangent_control | abs(angle_secd_fast_cmp) > tangent_control)                                                     
                                                                        if(abs(angle_fast)+control_vecter_angle > abs(atan((xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2)-fast_total_coor{1,inside_loop}(fast_obj_start,2) )/(xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1)-fast_total_coor{1,inside_loop}(fast_obj_start,1)))*180/pi) & abs(angle_fast)-control_vecter_angle < abs(atan((xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2)-fast_total_coor{1,inside_loop}(fast_obj_start,2) )/(xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1)-fast_total_coor{1,inside_loop}(fast_obj_start,1)))*180/pi)) % unit is degree                       
                                                                            pass_numfil=1;
                                                                        end
                                                                    else
                                                                        if(angle_fast+control_vecter_angle > atan((xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2)-fast_total_coor{1,inside_loop}(fast_obj_start,2) )/(xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1)-fast_total_coor{1,inside_loop}(fast_obj_start,1)))*180/pi & angle_fast-control_vecter_angle < atan((xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2)-fast_total_coor{1,inside_loop}(fast_obj_start,2) )/(xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1)-fast_total_coor{1,inside_loop}(fast_obj_start,1)))*180/pi) % unit is degree  
                                                                            pass_numfil=1;
                                                                        end
                                                                    end
                                                                else
                                                            ...
                                                            if(pass_numfil == 1) 
                                                                ...
                                                                % finish third
                                                                distance_thrd_mat=0;
                                                                [row_f3,col_f3,check_f3]=find( xyz{1,fast_obj_start+3}(:,1) < xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1) + distance_threshold_fast & xyz{1,fast_obj_start+3}(:,1) > xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1) - distance_threshold_fast);       
                                                                if(check_f3 == 1)
                                                                    maxsize_fast_xyz3=size(row_f3);
                                                                    for dist_thrd_loop = 1: maxsize_fast_xyz3(1,1)                                   
                                                                        xx_3 = xyz{1,fast_obj_start+3}(row_f3(dist_thrd_loop),1) - xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1);
                                                                        yy_3 = xyz{1,fast_obj_start+3}(row_f3(dist_thrd_loop),2) - xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2);
                                                                        distance_thrd_mat(row_f3(dist_thrd_loop),1) = sqrt((xx_3^2)+(yy_3^2));
                                                                        if( xx_3 == 0 & yy_3 == 0)
                                                                            distance_thrd_mat(row_f3(dist_thrd_loop),1) = 0.1;
                                                                        end
                                                                    end
                                                                   [row_d3]=find(distance_thrd_mat==0);
                                                                    size_row_d3=size(row_d3)
                                                                    for index=1:size_row_d3(1,2)
                                                                        distance_thrd_mat(row_d3(index),1)=nan;
                                                                    end
                                                                    [row_back3]=find(distance_thrd_mat==0.1);
                                                                    size_row_back3=size(row_back3)
                                                                    for index=1:size_row_back3(1,2)
                                                                        distance_thrd_mat(row_back3(index),1)=0;
                                                                    end
                                                                    ...
                                                                    if (check_third == 1)
                                                                        ...
                                                                        for find_rest_loop = 1: row_third_size(1,1)
                                                                            ...
                                                                            if( (xyz{1,fast_obj_start+3}(row_third(find_rest_loop),2)-xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2)) == 0 & (xyz{1,fast_obj_start+3}(row_third(find_rest_loop),1)-xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1)) ==0 )
                                                                                angle_rest_th_test=0;
                                                                            end
                                                                            ...
                                                                            if( priority_angle_order3(find_rest_loop,1) == 0)
                                                                            ...
                                                                            if(stop_search == 0)
                                                                                ...
                                                                                if(posit_nega == posit_nega2)
                                                                                    ...
                                                                                    if(increase_check == 0)  
                                                                                        ...
                                                                                        if( (xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),2)-xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2)) == 0 & (xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),1)-xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1))==0)
                                                                                            angle_rest_th_cmp=0;
                                                                                        end
                                                                                        if( dist_thresh_third > decide_slow_angle)
                                                                                            if(abs(angle_secd_fast)>tangent_control | abs(angle_rest_th_cmp)>tangent_control)    
                                                                                                if(abs(angle_secd_fast)+control_vecter_angle > abs(atan((xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),2)-xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),2) )/(xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),1)-xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1)))*180/pi) & abs(angle_secd_fast)-control_vecter_angle < abs(atan((xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),2)-xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),2) )/(xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),1)-xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1)))*180/pi))  
                                                                                                    pass_numfil=1;
                                                                                                end
                                                                                            else
                                                                                                if(angle_secd_fast+control_vecter_angle > atan((xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),2)-xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),2) )/(xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),1)-xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1)))*180/pi & angle_secd_fast-control_vecter_angle < atan((xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),2)-xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),2) )/(xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),1)-xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1)))*180/pi)  
                                                                                                    pass_numfil=1;
                                                                                                end
                                                                                            end
                                                                                        else
                                                                                        ...
                                                                                        if( pass_numfil == 1) 
                                                                                            ...
                                                                                            [row_f4,col_f4,check_f4]=find( xyz{1,fast_obj_start+4}(:,1) < xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),1) + distance_threshold_fast & xyz{1,fast_obj_start+4}(:,1) > xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),1) - distance_threshold_fast);       
                                                                                            if(check_f4 == 1)
                                                                                                distance_four_mat=0;
                                                                                                maxsize_fast_xyz4=size(row_f4);
                                                                                                for dist_four_loop = 1: maxsize_fast_xyz4(1,1)
                                                                                                    xx_4 = xyz{1,fast_obj_start+4}(row_f4(dist_four_loop),1) - xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),1);    
                                                                                                    yy_4 = xyz{1,fast_obj_start+4}(row_f4(dist_four_loop),2) - xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),2);     
                                                                                                    distance_four_mat(row_f4(dist_four_loop),1) = sqrt((xx_4^2)+(yy_4^2));                           
                                                                                                    if( xx_4 == 0 & yy_4 == 0)
                                                                                                        distance_four_mat(row_f4(dist_four_loop),1) = 0.1;
                                                                                                    end
                                                                                                end
                                                                                                [row_d4]=find(distance_four_mat==0);
                                                                                                size_row_d4=size(row_d4)
                                                                                                for index=1:size_row_d4(1,2)
                                                                                                    distance_four_mat(row_d4(index),1)=nan;
                                                                                                end
                                                                                                [row_back4]=find(distance_four_mat==0.1);
                                                                                                size_row_back4=size(row_back4)
                                                                                                for index=1:size_row_back4(1,2)
                                                                                                    distance_four_mat(row_back4(index),1)=0;
                                                                                                end
                                                                                                ...
                                                                                                if (check_four == 1)
                                                                                                    ...
                                                                                                    for find_four_loop = 1: row_four_size(1,1)
                                                                                                        ...
                                                                                                        if( (xyz{1,fast_obj_start+4}(row_four(find_four_loop),2)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),2)) == 0 & (xyz{1,fast_obj_start+4}(row_four(find_four_loop),1)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),1)) == 0)
                                                                                                            angle_final_th_test=0;
                                                                                                        end
                                                                                                        ...
                                                                                                        if ( priority_angle_order4(find_four_loop,1) == 0)
                                                                                                        ...
                                                                                                        if(stop_search == 0)
                                                                                                            ...
                                                                                                            if(posit_nega == posit_nega2)
                                                                                                                ...
                                                                                                                if(increase_check == 0)                                                                               
                                                                                                                    ...
                                                                                                                    if( (xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),2)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),2)) == 0 & (xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),1)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),1)) ==0 )
                                                                                                                    ...
                                                                                                                    if( dist_rest_th > decide_slow_angle)
                                                                                                                        if(abs(angle_rest_th)>tangent_control | abs(angle_final_th_cmp) >tangent_control)
                                                                                                                            if(abs(angle_rest_th)+control_vecter_angle > abs(atan((xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),2)-xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2) )/(xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),1)-xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1)))*180/pi) & abs(angle_rest_th)-control_vecter_angle < abs(atan((xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),2)-xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2) )/(xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),1)-xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1)))*180/pi))
                                                                                                                                pass_numfil=1;
                                                                                                                            end
                                                                                                                        else
                                                                                                                            if(angle_rest_th+control_vecter_angle > atan((xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),2)-xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2) )/(xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),1)-xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1)))*180/pi & angle_rest_th-control_vecter_angle < atan((xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),2)-xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2) )/(xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),1)-xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1)))*180/pi)
                                                                                                                                pass_numfil=1;
                                                                                                                            end
                                                                                                                        end
                                                                                                                    else  
                                                                                                                         if(abs(angle_rest_th)>tangent_control | abs(angle_final_th_cmp) >tangent_control)
                                                                                                                            if(abs(angle_rest_th)+control_vecter_angle*multilple_short_angle > abs(atan((xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),2)-xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2) )/(xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),1)-xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1)))*180/pi) & abs(angle_rest_th)-control_vecter_angle*multilple_short_angle  < abs(atan((xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),2)-xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2) )/(xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),1)-xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1)))*180/pi))
                                                                                                                                pass_numfil=1;
                                                                                                                            end
                                                                                                                        else
                                                                                                                            if(angle_rest_th+control_vecter_angle*multilple_short_angle  > atan((xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),2)-xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2) )/(xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),1)-xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1)))*180/pi & angle_rest_th-control_vecter_angle*multilple_short_angle  < atan((xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),2)-xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2) )/(xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),1)-xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1)))*180/pi)
                                                                                                                                pass_numfil=1;
                                                                                                                            end
                                                                                                                        end
                                                                                                                    end
                                                                                                                    if( pass_numfil == 1)
                                                                                                                        ...
                                                                                                                        if(fast_obj_start == 1)
                                                                                                                            ...
                                                                                                                            distance_six_mat=0;
                                                                                                                            [row_f5,col_f5,check_f5]=find( xyz{1,fast_obj_start+4}(:,1) < xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),1) + distance_threshold_fast & xyz{1,fast_obj_start+4}(:,1) > xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),1) - distance_threshold_fast);       
                                                                                                                            if(check_f5 == 1)
                                                                                                                                stop_search2=0;
                                                                                                                                maxsize_fast_xyz6=size(row_f5);
                                                                                                                                if( dist_final_th > decide_slow)
                                                                                                                                    for dist_six_loop = 1: maxsize_fast_xyz6(1,1)                                   
                                                                                                                                        xx_6 = xyz{1,fast_obj_start+4}(row_f5(dist_six_loop),1) - xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),1);
                                                                                                                                        yy_6 = xyz{1,fast_obj_start+4}(row_f5(dist_six_loop),2) - xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),2);
                                                                                                                                        distance_six_mat(row_f5(dist_six_loop),1) = sqrt((xx_6^2)+(yy_6^2));                                                                    
                                                                                                                                        if( xx_6 == 0 & yy_6 == 0)
                                                                                                                                            distance_six_mat(row_f5(dist_six_loop),1) = 0.1;
                                                                                                                                        end
                                                                                                                                    end
                                                                                                                                    [row_d6]=find(distance_six_mat==0);
                                                                                                                                    size_row_d6=size(row_d6)
                                                                                                                                    for index=1:size_row_d6(1,2)
                                                                                                                                        distance_six_mat(row_d6(index),1)=nan;
                                                                                                                                    end
                                                                                                                                    [row_back6]=find(distance_six_mat==0.1);
                                                                                                                                    size_row_back6=size(row_back6)
                                                                                                                                    for index=1:size_row_back6(1,2)
                                                                                                                                        distance_six_mat(row_back6(index),1)=0;
                                                                                                                                    end
                                                                                                                                    if(dist_final_th <= decide_slow)
                                                                                                                                        [row_six,col_six,check_six]=find(distance_six_mat < distance_threshold_fast );
                                                                                                                                    else
                                                                                                                                        [row_six,col_six,check_six]=find(distance_six_mat < dist_final_th*threshold_diff_dislocation );
                                                                                                                                    end
                                                                                                                                    if (check_six == 1)
                                                                                                                                        ...
                                                                                                                                        for find_six_loop = 1: row_six_size(1,1)
                                                                                                                                            angle_six_th_test=atan((xyz{1,fast_obj_start+4}(row_six(find_six_loop),2)-xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),2))/(xyz{1,fast_obj_start+4}(row_six(find_six_loop),1)-xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),1)))*180/pi;                                                           
                                                                                                                                            if( (xyz{1,fast_obj_start+4}(row_six(find_six_loop),2)-xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),2)) == 0 & (xyz{1,fast_obj_start+4}(row_six(find_six_loop),1)-xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),1)) )
                                                                                                                                                angle_six_th_test=0;
                                                                                                                                            end
                                                                                                                                            ...
                                                                                                                                            if( priority_angle_order6(find_six_loop,1) == 0)
                                                                                                                                        ...
                                                                                                                                        for find_six_loop2 = 1: row_six_size(1,1)
                                                                                                                                            ...
                                                                                                                                            if(stop_search2 == 0)
                                                                                                                                                ...
                                                                                                                                                if(posit_nega == posit_nega2)
                                                                                                                                                    increase_check=1; 
                                                                                                                                                    if((distance_six_mat(row_six(priority_angle_order6(find_six_loop2,2)),1)+dist_final_th)*increase_distance_threshold < sqrt( (xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),1)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),1))^2+(xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),2)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),2))^2 ))
                                                                                                                                                        increase_check=0;                                                     
                                                                                                                                                    else            
                                                                                                                                                        increase_check=1;
                                                                                                                                                    end  
                                                                                                                                                    if(increase_check == 0)
                                                                                                                                                        angle_six_th_cmp= atan((xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),2)-xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),2))/(xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),1)-xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),1)))*180/pi;
                                                                                                                                                        if( (xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),2)-xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),2)) == 0 & (xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),1)-xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),1)) == 0 )
                                                                                                                                                            angle_six_th_cmp=0;
                                                                                                                                                        end
                                                                                                                                                        if( dist_final_th > decide_slow_angle)
                                                                                                                                                            if(abs(angle_final_th)>tangent_control | abs(angle_six_th_cmp)>tangent_control)    
                                                                                                                                                                if(abs(angle_final_th)+control_vecter_angle > abs(atan( (xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),2)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),2) )/(xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),1)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),1)) )*180/pi) & abs(angle_final_th)-control_vecter_angle < abs(atan( (xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),2)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),2) )/(xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),1)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),1)) )*180/pi))  
                                                                                                                                                                    pass_numfil=1;
                                                                                                                                                                end
                                                                                                                                                            else
                                                                                                                                                                if(angle_final_th+control_vecter_angle > atan( (xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),2)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),2) )/(xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),1)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),1)) )*180/pi & angle_final_th-control_vecter_angle < atan( (xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),2)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),2) )/(xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),1)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),1)) )*180/pi)  
                                                                                                                                                                    pass_numfil=1;
                                                                                                                                                                end
                                                                                                                                                            end
                                                                                                                                                        else
                                                                                                                                                            if(abs(angle_final_th)>tangent_control | abs(angle_six_th_cmp)>tangent_control)
                                                                                                                                                                if(abs(angle_final_th)+control_vecter_angle*multilple_short_angle > abs(atan( (xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),2)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),2) )/(xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),1)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),1)) )*180/pi) & abs(angle_final_th)-control_vecter_angle*multilple_short_angle < abs(atan( (xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),2)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),2) )/(xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),1)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),1)) )*180/pi))  
                                                                                                                                                                    pass_numfil=1;
                                                                                                                                                                end
                                                                                                                                                            else
                                                                                                                                                                if(angle_final_th+control_vecter_angle*multilple_short_angle > atan( (xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),2)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),2) )/(xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),1)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),1)) )*180/pi & angle_final_th-control_vecter_angle*multilple_short_angle < atan( (xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),2)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),2) )/(xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),1)-xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),1)) )*180/pi)  
                                                                                                                                                                    pass_numfil=1;
                                                                                                                                                                end
                                                                                                                                                            end
                                                                                                                                                        end
                                                                                                                            ...
                                                                                                                            fst_mv_oj_coor(1,1) = xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),1);
                                                                                                                            fst_mv_oj_coor(1,2) = xyz{1,fast_obj_start+1}(row_dist(priority_angle_order(try_loop2,2)),2);       
                                                                                                                            fst_mv_oj_coor(1,3) = inside_loop;

                                                                                                                            fst_mv_oj_coor(2,1) = xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),1);
                                                                                                                            fst_mv_oj_coor(2,2) = xyz{1,fast_obj_start+2}(row_secd(priority_angle_order2(find_secd_loop2,2)),2);          
                                                                                                                            fst_mv_oj_coor(2,3) = inside_loop;

                                                                                                                            fst_mv_oj_coor(3,1) = xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),1);
                                                                                                                            fst_mv_oj_coor(3,2) = xyz{1,fast_obj_start+3}(row_third(priority_angle_order3(find_rest_loop2,2)),2); 
                                                                                                                            fst_mv_oj_coor(3,3) = inside_loop;

                                                                                                                            fst_mv_oj_coor(4,1) = xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),1);
                                                                                                                            fst_mv_oj_coor(4,2) = xyz{1,fast_obj_start+4}(row_four(priority_angle_order4(find_four_loop2,2)),2);
                                                                                                                            ...
                                                                                                                            if(check_f5 == 1)
                                                                                                                                ...
                                                                                                                                if( dist_final_th > decide_slow)                                                                                                            
                                                                                                                                    ...
                                                                                                                                    for dist_six_loop = 1: maxsize_fast_xyz6(1,1)                                   
                                                                                                                                        ...
                                                                                                                                        if( xx_6 == 0 & yy_6 == 0)
                                                                                                                                            distance_six_mat(dist_six_loop,1) = 0.1;
                                                                                                                                        end
                                                                                                                                    end     
                                                                                                                                    [row_d6]=find(distance_six_mat==0);
                                                                                                                                    size_row_d6=size(row_d6)
                                                                                                                                            ...
                                                                                                                                            if(stop_search2 == 0)
                                                                                                                                                ...
                                                                                                                                                if(posit_nega == posit_nega2)
                                                                                                                                                    ...
                                                                                                                                                    if(increase_check == 0)
                                                                                                                                                        ...
                                                                                                                                                        if( pass_numfil == 1)
                                                                                                                                                            fst_mv_oj_coor(5,1) = xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),1);
                                                                                                                                                            fst_mv_oj_coor(5,2) = xyz{1,fast_obj_start+4}(row_six(priority_angle_order6(find_six_loop2,2)),2);
                                                                                                                                                            stop_search2=1;
                                                                                                                                                        end
                                                                                                                                                    end
                                                                                                                                                end
                                                                                                                            ...
                                                                                                                            if( prevent_rest_check{1,1} == 0)
                                                                                                                                ...
                                                                                                                                prevent_rest_check{1,inside_loop}(1,1) = fst_mv_oj_coor(1,1);
                                                                                                                                prevent_rest_check{1,inside_loop}(1,2) = fst_mv_oj_coor(1,2); 
                                                                                                                                prevent_rest_check{1,inside_loop}(2,1) = fst_mv_oj_coor(2,1);
                                                                                                                                prevent_rest_check{1,inside_loop}(2,2) = fst_mv_oj_coor(2,2); 
                                                                                                                                prevent_rest_check{1,inside_loop}(3,1) = fst_mv_oj_coor(3,1);
                                                                                                                                prevent_rest_check{1,inside_loop}(3,2) = fst_mv_oj_coor(3,2); 
                                                                                                                                prevent_rest_check{1,inside_loop}(4,1) = fst_mv_oj_coor(4,1);
                                                                                                                                prevent_rest_check{1,inside_loop}(4,2) = fst_mv_oj_coor(4,2);
                                                                                                                                ...
                                                                                                                                if(stop_search2==1)
                                                                                                                                    ...
                                                                                                                                    prevent_rest_check{1,inside_loop}(5,1)= fst_mv_oj_coor(5,1);
                                                                                                                                    prevent_rest_check{1,inside_loop}(5,2)= fst_mv_oj_coor(5,2);
                                                                                                                                end
                                                                                                                                stop_search=1; %prevent duplicate trajectories
