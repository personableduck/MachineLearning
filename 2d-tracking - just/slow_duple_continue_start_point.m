function ans_xyz= slow_duple_continue_start_point(origin_size_fst,xyz_11,fast_total_coor,fast_obj_start,duple_mat,increase_number,distance_threshold_fast,priority_degree_rate,threshold_diff_dislocation,priority_zero,decide_slow,decide_slow_angle,tangent_control,control_vecter_angle,multilple_short_angle,increase_distance_threshold)
...
if( estm_ft_size(1,1) >= origin_size_fst )
    [row_f1,col_f1,check_f1]=find( xyz_11{1,fast_obj_start+increase_number}(:,1) < fast_total_coor{1,duple_mat}(origin_size_fst ,1) + distance_threshold_fast & xyz_11{1,fast_obj_start+increase_number}(:,1) > fast_total_coor{1,duple_mat}(origin_size_fst,1) - distance_threshold_fast);
...
if(check_f1 == 1)  
    ...
    if (check_dist == 1) %first distance threshold check
        ...
        for try_loop = 1: row_dist_size(1,1) 
            angle_fast_test = atan((xyz_11{1,fast_obj_start+increase_number}(distance_fast_mat(row_dist(try_loop),2),2)-yc_back_1)/(xyz_11{1,fast_obj_start+increase_number}(distance_fast_mat(row_dist(try_loop),2),1)-xc_back_1))*180/pi; % unit is degree   
            ...
            if( (xyz_11{1,fast_obj_start+increase_number}(distance_fast_mat(row_dist(try_loop),2),2)-yc_back_1) == 0 & (xyz_11{1,fast_obj_start+increase_number}(distance_fast_mat(row_dist(try_loop),2),1)-xc_back_1) == 0 )
